...
export default function Team() {
  return (
    <section id="team" className="bg-bg text-cream">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-3 md:gap-0">
            <Reveal y={70} delay={0.05}>
              <h2 className="mega leading-[0.88] text-cream">
                A Culture<br />
                <span className="text-cream/35">of—&nbsp;Care</span>
              </h2>
            </Reveal>
            <Reveal delay={0.08}>
              <p className="max-w-[280px] md:mb-3 font-body text-[0.95rem] leading-[1.85] text-cream/70">
        ...
        <Stagger className="mt-8 grid gap-5 md:mt-10 md:grid-cols-3" y={48} gap={0.12}>
          {TEAM.map((person) => (
            <div key={person.name} data-stagger className="group">
              {/* Photo */}
              <div className="relative aspect-[3/3.2] overflow-hidden bg-surface">
                <Img
                  src={person.photo}
                ...
                />
                {/* Role badge */}
                <span className="absolute left-3 top-3 bg-bg px-3 py-1 label text-cream/60">
                  {person.role}
                </span>
              </div>
              ...
              <div className="mt-3 border-t border-cream/[0.06] pt-3">
                <h3 className="display text-[1.4rem] text-cream">{person.name}</h3>
                <p className="mt-2 text-[0.85rem] leading-relaxed text-cream/35">
                  {person.note}
                </p>
              </div>
            </div>
